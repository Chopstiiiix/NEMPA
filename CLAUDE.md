...
- ⚠️ **Auth emails come from Supabase, not Sparrowtell.** The built-in email service sends from `noreply@mail.app.supabase.io` and its sender name/address cannot be changed — that needs custom SMTP (Project Settings → Authentication → SMTP) with a provider (Resend/Postmark/SendGrid) on a domain you own. The built-in sender is also rate-limited to a handful of emails per hour, so it is **not usable at launch** regardless of branding. Email Templates can be branded for free in the meantime, but the From address stays Supabase.
